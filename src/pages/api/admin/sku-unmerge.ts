...
export const GET: APIRoute = async (context) => {
  ...
  try {
    ...
    if (!sourceCode) return new Response(JSON.stringify({ error: 'sourceCode is required' }), { status: 400 });

    const preview = await previewSkuUnmerge(db, sourceCode);
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    const body = await context.request.json<{ sourceCode: string }>();

    const result = await unmergeSku(db, user.id, body.sourceCode);
