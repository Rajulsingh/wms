    ...
    const rows = await db
      .prepare(
        `SELECT ee.id, ee.type, ee.notes, ee.resolved, ee.created_at, u.name AS user_name,
         ...
         WHERE (COALESCE(o1.warehouse_id, o2.warehouse_id, o3.warehouse_id) = ? OR (o1.id IS NULL AND o2.id IS NULL AND o3.id IS NULL))
           AND (? = 1 OR ee.resolved = 0)
         ORDER BY ee.resolved ASC, ee.created_at DESC
         LIMIT 300`
      )
      .bind(warehouseId, includeResolved ? 1 : 0)
      .all();
