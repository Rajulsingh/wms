...
export const GET: APIRoute = async (context) => {
  ...
  try {
    ...
    const rows = await db
      .prepare(`SELECT id, name, role, active, created_at FROM users WHERE warehouse_id = ? ORDER BY active DESC, role, name`)
      .bind(warehouseId)
      .all();
...
/** Deactivate/reactivate — never a hard delete, so audit history (which references user_id) stays intact. */
export const PATCH: APIRoute = async (context) => {
  const db = getDb();
  try {
    const requester = await requireUser(context, db, ['admin']);
    const body = await context.request.json<{ userId: string; active: boolean }>();
